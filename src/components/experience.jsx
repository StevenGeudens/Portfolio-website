        ...
        <li ref={ref} className="my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col items-center justify-between md:w-[80%]">
            ...
            <motion.div initial={{y:50}} whileInView={{y:0}} transition={{duration: 0.5, type: "spring"}}>
                ...
                <span className="capitalize font-medium text-dark/75 dark:text-light/75 xs:text-sm">
                    {new Date(startDate).toLocaleString("en-US", { year: 'numeric', month: 'long', day: 'numeric' })} - {endDate ? new Date(endDate).toLocaleString("en-US", { year: 'numeric', month: 'long', day: 'numeric' }) : "Present"} | {address}
                </span>
        ...
        <div className="my-64">
            ...
            <div ref={ref} className="w-[75%] mx-auto relative lg:w-[90%] md:w-full">
                ...
                <ul className="w-full flex flex-col items-start justify-between ml-4 xs:ml-2">
                    {experiences.map(e => <Details key={e.id} {...e}/>)}
                </ul>
            </div>
        </div>
    )
